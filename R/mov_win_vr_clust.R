...
fit_ctrl1 <- stan(
  file = readRDS(paste0(family,"_null.stan")),
  data = dat_stan,
  pars = c('alpha', 'y_sd', 
           'yhat', 'log_lik'),
...
fit_yr1 <- stan(
  file = readRDS(paste0(family,"_yr.stan")),
  data = dat_stan,
...
fit_gaus <- stan(
  file = readRDS(paste0(family,"_gaus.stan")),
  data = dat_stan,
...
fit_expp <- stan(
  file = readRDS(paste0(family,"_expp.stan")),
  data = dat_stan,
  pars = c('sens_mu', 'sens_sd', 'alpha', 'beta', 'y_sd', 
...
fit_mb_h <- stan(
  file = readRDS(paste0(family,"_movbeta_h.stan")),
  data = dat_stan,
...
fit_mb <- stan(
  file = readRDS(paste0(family,"_movbeta.stan")),
  data = dat_stan,
...
# moving beta hierarchical
fit_mb_h_n <- stan(
  file = readRDS(paste0(family,"_movbeta_h_nest.stan")),
  data = dat_stan,
...
# moving beta correlated
fit_mb_n <- stan(
  file = readRDS(paste0(family,"_movbeta_nest.stan")),
  data = dat_stan,
...
# Simplex nested
fit_24_nest <- stan(
  file = readRDS(paste0(family,"_dirichlet_nest.stan")),
  data = dat_stan,
...
fit_gaus_nest <- stan(
  file = readRDS(paste0(family,"_gaus_nest.stan")),
  data = dat_stan,
...
fit_expp_nest <- stan(
  file = readRDS(paste0(family,"_expp_nest.stan")),
  data = dat_stan,
  pars = c('sens_mu', 'sens_sd', 'theta_y', 'alpha', 'beta', 'y_sd', 
           'yhat', 'log_lik'),
  warmup = sim_pars$warmup,
...
CrossVal <- function(i, mod_data, response) {       # i is index for row to leave out
  ...
  mod_null   <- stan_model(file = paste0("code/stan/",family,"_null_crossval.rds") )
  mod_null   <- readRDS(paste0("code/stan/",family,"_null_crossval.rds")) 
  
  
  # compile all models first
  mod_null   <- readRDS(paste0(family,"_null_crossval.rds") )
  mod_yr     <- readRDS(paste0(family,"_yr_crossval.stan") )
  mod_gaus   <- readRDS(paste0(family,"_gaus_crossval.stan")  )
  mod_expp   <- readRDS(paste0(family,"_expp_crossval.stan") )
  mod_mb_h   <- readRDS(paste0(family,"_movbeta_h_crossval.stan") )
  mod_mb     <- readRDS(paste0(family,"_movbeta_crossval.stan") )
  mod_mb_h_n <- readRDS(paste0(family,"_movbeta_h_nest_crossval.stan") )
  mod_mb_n   <- readRDS(paste0(family,"_movbeta_nest_crossval.stan") )
  mod_gaus_n <- readRDS(paste0(family,"_gaus_nest_crossval.stan") )
  mod_expp_n <- readRDS(paste0(family,"_expp_nest_crossval.stan") )
  mod_smpl_n <- readRDS(paste0(family,"_dirichlet_nest_crossval.stan") )
...
# mean squared error
mse <- cxval_pred %>% 
          dplyr::select(gev_pred:expp_n_pred) %>%
...
# Expected Log Predictive Density
elpd <- cxval_pred %>% 
          dplyr::select(gev_elpd:expp_n_elpd) %>%
          apply(2, sum) %>% 
          as.matrix %>% 
          as.data.frame %>%
