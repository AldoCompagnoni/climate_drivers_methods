...
fit_simpl1 <- sampling(
  object = readRDS(paste0(family,"_dirichlet.RDS")),
  data = dat_stan,
...
dat_stan$clim   <- mod_data$climate[,1:12]
fit_ridge1 <- sampling(
  object = readRDS(paste0(family,"_horse.RDS")),
  data = dat_stan,
...
CrossVal <- function(i, mod_data, response){       # i is index for row to leave out
  ...
  fit_simpl1_crossval <- sampling(
    object = readRDS(paste0(family,"_dirichlet_crossval.RDS")),
    data = dat_stan_crossval,
    pars = c('theta',  'alpha', 'beta', 'y_sd', 
             'pred_y', 'log_lik_test'),
    warmup = sim_pars$warmup,
  ...
  fit_ridge1_crossval <- sampling(
    object = readRDS(paste0(family,"_horse_crossval.RDS")),
    data = dat_stan_crossval,
    pars = c('alpha',  'beta', 'y_sd', 
             'pred_y', 'log_lik_test'),
    warmup = sim_pars$warmup,
...
write.csv(cxval_post, paste0(out_dir,"-",spp_num,"crossval_pred_post_",spp_name,"_",response,"_",clim_var,".csv"), row.names = F)
write.csv(diag_df,    paste0(out_dir,"-",spp_num,"_diagnostics_",spp_name,"_",response,"_",clim_var,".csv"), row.names = F)
save.image( paste0(out_dir,'_workshace_',spp_num,'.RData') )
