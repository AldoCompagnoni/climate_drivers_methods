...
null_prior <- data.frame( 
  alpha = rnorm(  it, 0,     alpha_sd) %>% boot::inv.logit(),
...
yhat_calc   <- function( a, b, clim_m) (a + b * clim_m) %>% inv_logit
...
yhat_calc   <- function( a, b, i, ante_x) inv_logit(a + b * ante_x[i,])
...
ggplot(sim_df, aes(y_sim) ) + 
  ...
  ggsave( 'results/prior_pc/beta_y_sim.tiff',
