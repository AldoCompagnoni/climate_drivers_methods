...
w_v    <- c( (pdens / sum(pdens)) * 0.2,
             ...
             (pdens / sum(pdens)) * 0.1 )
# climate "data"
clim_1 <- select(anom_a, -year) %>% as.matrix

# simulate data
sim_data <- function(beta_x, rep){
  
  x      <- clim_1 %*% w_v

  # replicate data
  rep_sim <- function(ii,beta_x){
  ...
  lapply(1:rep, rep_sim, beta_x) %>% bind_rows

}

# plot it
plot(y ~ x, data=sim_data(0.2, rep))


clim_m <- lapply(1:rep, function(x) clim_1 ) %>% 
...
mod_l$'0.2'  <- fit_mods( sim_data(0.2, rep) ) %>% model_names
mod_l$'0.45' <- fit_mods( sim_data(0.45, rep) ) %>% model_names
mod_l$'0.7'  <- fit_mods( sim_data(0.7, rep) ) %>% model_names
mod_l$'1.2'  <- fit_mods( sim_data(1.2, rep) ) %>% model_names
mod_l$'1.7'  <- fit_mods( sim_data(1.7, rep) ) %>% model_names
mod_l$'2.2'  <- fit_mods( sim_data(2.2, rep) ) %>% model_names
